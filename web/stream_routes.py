...
import mimetypes
import time
from info import BIN_CHANNEL as DEFAULT_BIN
...
async def root_route_handler(request):
    raise web.HTTPFound('https://google.com')

@routes.get("/ping")
async def ping_handler(request):
    uptime = get_readable_time(int(time.time() - temp.start_time))
    return web.json_response({"status": "running", "uptime": uptime})
...
async def watch_handler(request):
    try:
        ...
        return web.Response(text=e, content_type='text/html')
